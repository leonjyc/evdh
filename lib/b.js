/* b.js, b: base js for evdh: EisF Video Download Helper, sceext <sceext@foxmail.com> 2009EisF2015, 2015.02 
...
/* functions */
	...
	function get_file_size(fpath, callback) {	// callback(error, size);
	...
	function write_file(fpath, data_buffer, callback) {	// finish callback(error);
		fs.writeFile(fpath, data_buffer, function(err){
			if (err) {
				callback(err);
			} else {
				callback(null);
