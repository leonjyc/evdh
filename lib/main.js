/* main.js, used in node.js, main js part for evdh : EisF Video Download Helper, sceext <sceext@foxmail.com> 2009EisF2015, 2015.01 
 * version 0.1.2.0 test201502031627 (public zh-cn version) 
...
function load_config_next(_step, _host) {
	...
	case 1:	// first step
		...
		_host.oc.callback = function(err){
			if (err) {
				time_log('错误: 加载配置文件 \"' + _host.oc.config_file + '\" 失败 ! ');
				_host.error = err;
	...
	case 2:	// load config done
		...
		time_log('信息: 使用配置文件 \"' + _host.oc.config_file + '\" ');
...
function load_token_next(_step, _host) {
	...
	case 1:	// first step
		...
		_b.get_file_size(_host.token_file, function(err, size){
			if (err) {
				time_log('错误: 无法访问 token 文件 \'' + _host.token_file + '\' ! ');
				_host.error = err;
	...
	case 2:	// got token file size
		...
		if (_host.token_file_size > 256) {	// more than 256 Byte, too big
			time_log('错误: token 文件太大 ! (' + _host.token_file_size + ' Byte) ');
		...
		_host.fr.callback = function(err){
			if (err) {
				time_log('错误: 无法读取 token 文件 \"' + _host.token_file + '\" ! ');
				_host.error = err;
	...
	case 3:	// got token from file
		...
		if (_b.check_char16(token) || (token.length < 8)) {	// token error
			time_log('错误: token 错误: 检查 长度为 ' + token.length + ' 的 token, 在 token 文件 \"' + _host.token_file + '\" 中, token 格式错误 ! 请在 token 文件中写入一个正确的 token. ');
...
function select_q_next(_step, _host) {
	...
	case 2:	// more than one video, ask user to select
		...
		time_log('信息: 有 ' + _host.now_set.length + ' 个视频, 视频质量 ' + _.ci.video_quality + ' (' + _host.fq + ') \n');
	...
	case 3:	// got user input
		...
		if ((n_video >= 0) && (n_video < _host.now_set.length)) {
			...
			time_log('信息: 你选择了 视频 ' + n_video + ' : ');
			
			show_video_info([_host.ni]);
			
			_next(0, _host);
		} else {	// error input
			time_log('错误: 输入错误, 没有这个视频 ! ');
...
function merge_video(info, callback) {	// finish callback(exit_code);
	...
	out_name = info.base_path + '/' + out_name;
	
	var merge_args = [tmp_path, out_name].concat(file_list);
	
	// make log
	var merge_log = _.ci.log_path + '/merge_video.log';
...
function make_merge_file_list(info, tmp_path) {
	...
	for (var i = 0; i < info.list.length; i++) {
		...
		// add to list
		name.push(path.relative(tmp_path, bp + '/' + p));
	}
