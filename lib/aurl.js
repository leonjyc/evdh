 ...
 * version 0.1.4.0 test201502150045 (public zh-cn version) 
...
function aurl_next(_step, _host) {
	...
	case 1:	// first step
		...
		time_log('INFO: 使用 token, 长度为 ' + etc.token.length);
		...
		// print info
		time_log('INFO: 正在 从 \"' + etc.analyse_request_website + '\" 获取信息 ... ');
	...
	case 2:	// got result
		...
		_host.ao.callback = function(err){
			if (err) {
				time_log('ERROR: 解析 接收到的 xml 文本失败 ! ');
		...
		time_log('INFO: 正在解析 接收到的 xml 文本 ... ');
