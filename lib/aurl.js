/* aurl.js, aurl: analyse url part for evdh: EisF Video Download Helper, sceext <sceext@foxmail.com> 2009EisF2015, 2015.02 
 * version 0.1.2.0 test201502142022 (public version) 
...
function aurl(url, callback) {	// finish callback(error, info);
	...
	// static config
	_host.result_file = path.join(etc.log_path, etc.result_file);
	_host.header_file = path.join(etc.log_path, etc.header_file);
...
function aurl_next(_step, _host) {
	...
	case 1:	// first step
		...
		time_log('INFO: use token of length ' + etc.token.length);
		...
		// print info
		time_log('INFO: getting info from \"' + etc.analyse_request_website + '\" ... ');
	...
	case 2:	// got result
		...
		_host.ao.callback = function(err){
			if (err) {
				time_log('ERROR: analyse received xml failed ! ');
		...
		time_log('INFO: analysing recevied xml ... ');
	...
	case -1:	// error step
		...
		break;
	default:	// step error
		time_log('ERROR: aurl.js aurl_next: step error ! ');
	}
}
